...
export function TypingTest(props: TypingTestProps) {
  ...
  useEffect(() => {
    const handleGlobalKeyDown = (e: KeyboardEvent) => {
      // Escape â†’ blur input
      if (e.key === "Escape" && isFocused && inputRef.current) {
        inputRef.current.blur();
        return;
      }
      // R â†’ restart test (when not actively typing a word)
      if (
        (e.key === "r" || e.key === "R") &&
        typed.length === 0 &&
        !started
      ...
      // Re-focus on any key if blurred
  ...
  }, [isFocused, inputRef, onRestart, typed, started]);
      ...
      <div className="relative w-full">
        ...
        >
          ...
          >
            ...
            {/* WPM + Accuracy â€” gated by liveStats setting */}
            {liveStats && (
              <>
                <span className="tabular-nums">
                  <span className="font-bold text-foreground text-lg">
      ...
      >
        {mode === "zen" && started ? (
          ...
          <>
            ...
            <span className="mx-1.5 text-muted-foreground/20">Â·</span>
            <kbd className="rounded-[4px] bg-foreground/[0.06] px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground">
              esc
            </kbd>
