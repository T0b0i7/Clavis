...
export function useTypingTest({
  ...
  const defaultMode: TestMode = "time";
  const defaultTimeOption: TimeOption = 30;
  const defaultWordOption: WordOption = 25;
  const defaultQuoteLength: QuoteLength = "medium";
  const defaultPunctuation = false;
  const defaultNumbers = false;
  const defaultDifficulty: Difficulty | undefined = "easy";
  ...
  useEffect(() => {
    ...
    const m = storedMode ?? defaultMode;
    const to = storedTime ?? defaultTimeOption;
    const wo = storedWordOption ?? defaultWordOption;
  ...
  // ── Frozen stats (computed via effect, safe avec React 19 concurrent) ─────
